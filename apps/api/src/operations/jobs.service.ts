...
export class JobsService {
  ...
  create(user: AuthUser, input: CreateJobInput) {
    ...
    return this.db.tx(user, async (tx) => {
      ...
      await this.audit.record(tx, user, {
        ...
        after: { clientId: input.clientId, type: input.type, location: input.location, status: job.status },
      });
      return job;
    });
  }
  ...
  /**
   * Only never-started jobs can be removed, and even then they are archived rather than
   * deleted: the job number was issued and must remain accounted for. Anything that has
   * been in the field is cancelled instead, which keeps it visible.
   */
  ...
  assign(user: AuthUser, id: string, inspectorId: string) {
    return this.db.tx(user, async (tx) => {
      ...
      await this.audit.record(tx, user, {
        ...
        entityLabel: job.jobNumber,
        branchId: job.branchId,
        before: { assignedInspectorId: job.assignedInspectorId },
        after: { assignedInspectorId: inspectorId },
      });
      return this.load(tx, id);
    });
  }
  ...
  start(user: AuthUser, id: string) {
    return this.db.tx(user, async (tx) => {
      ...
      await this.transition(tx, user, job, 'in_progress');
      return this.load(tx, id);
    });
  }
  ...
  submit(user: AuthUser, id: string) {
    return this.db.tx(user, async (tx) => {
      ...
      await this.transition(tx, user, job, 'under_review', `submitted_at = now(), review_comment = NULL`);
      return this.load(tx, id);
    });
  }
  ...
  returnForRework(user: AuthUser, id: string, comment: string) {
    return this.db.tx(user, async (tx) => {
      const job = await this.load(tx, id, true);
      await this.transition(tx, user, job, 'in_progress', 'review_comment = $2', [comment]);
      return this.load(tx, id);
    });
  }
  ...
  approve(user: AuthUser, id: string) {
    return this.db.tx(user, async (tx) => {
      const job = await this.load(tx, id, true);
      await this.transition(tx, user, job, 'approved', 'approved_at = now(), approved_by = $2', [user.id]);
  ...
  cancel(user: AuthUser, id: string) {
    return this.db.tx(user, async (tx) => {
      const job = await this.load(tx, id, true);
      await this.transition(tx, user, job, 'cancelled');
      return this.load(tx, id);
    });
  }

  /**
   * The single gate every status change goes through: validate the transition, apply it, and
   * record it. A status change that is not in the audit log did not happen.
   */
  ...
  private assertAssignedInspector(user: AuthUser, job: InspectionJob) {
    ...
    if (user.scope === 'own' && job.assignedInspectorId !== user.id) {
