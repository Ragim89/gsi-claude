...
**Известное ограничение.** Если проба отправлена в лабораторию другого офиса, сотрудник принимающего офиса видит строку пробы и строку анализа по политике, но открыть карточку не может: и проба, и анализ читаются с `JOIN` к заявке и клиенту, а те принадлежат отправляющему офису и закрыты его политикой. Сегодня это значит, что межофисную отправку доводит до конца офис-владелец заявки. Открывать другому офису коммерческие данные клиента — решение не техническое, поэтому оно вынесено отдельно, а не сделано попутно.
...
Шаблон — это список секций и их настроек, а не код: семь типов документов — это семь строк в `report_templates`, а не семь экранов. Версию формы поднимает триггер базы, и каждая выпущенная ревизия помнит, какой версией её напечатали.
