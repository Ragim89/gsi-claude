...
## Уведомления (PHASE 10)

Не отдельный движок статусов — реакция на переходы, которые движки выше уже совершают. Каждый из них эмитит событие в `JobEventsService` (в `CoreModule` с PHASE 10, см. `docs/DATABASE.md`); `NotificationEventsListener` слушает и создаёт строку в `notifications` для нужных получателей.

| Событие | Кто узнаёт | Право получателя |
|---|---|---|
| Заявка назначена | назначенный сотрудник | — (адресат назван в самом событии) |
| Проба принята лабораторией (`sample.received_by_lab`) | сотрудники офиса-получателя | `sample.accept_lab` |
| Результат анализа отправлен на проверку (`test_request.under_review`) | сотрудники офиса | `lab.result.review` |
| Отчёт отправлен на проверку (`report.under_review`) | сотрудники офиса | `report.review` |
| Отчёт одобрен, ждёт выпуска (`report.approved`) | сотрудники офиса | `report.issue` |
| Отчёт выпущен | автор ревизии | — (адресат назван в самом событии) |
| Инспекция скоро (в ближайшие 24 часа) | назначенная на инспекцию команда | часовой сдвиг, не событие — `inspection_assignments` |
| Счёт просрочен | сотрудники офиса | `invoice.remind`; часовой сдвиг, не событие |

Получатели по праву — это все активные пользователи офиса, у чьей роли есть это право (`users_with_permission()`), а не одно избранное лицо: увидит уведомление и офис-менеджер, и супервайзер, если оба держат нужное право.
