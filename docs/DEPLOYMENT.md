...
- **Пробел RLS между офисами.** Проба, отправленная в лабораторию **другого** офиса (документированная возможность, `app_sees_laboratory`, `SECURITY.md`), становится невидимой для лаборатории на шагах `receive`/`accept`, если заявка/клиент принадлежат другому офису: `samples.service.ts#load` присоединяет `inspection_jobs`/`clients`, у которых собственная RLS-политика не знает об исключении для лаборатории, и `JOIN` тихо режет строку — API отвечает `404 Sample not found`. Обнаружено приёмочным прогоном PHASE 13 (`apps/api/test/acceptance.spec.ts`) впервые: ни один более ранний тест не сочетал заявку одного офиса с лабораторией другого. Исправление — изменение RLS-политик `inspection_jobs`/`clients` — вне рамок этой фазы (Docker/бэкап/CI), требует отдельного тестирования на регрессию доступа. **До исправления: не дожидаться этого сценария в production** — лаборатория должна принадлежать тому же офису, что и заявка, либо доступ к пробе временно даёт HQ-роль (`scope: global`), которая не зависит от этой политики.
- **Аудит счетов.** `invoice.create`/`invoice.issue`/`invoice.pay` не пишут в `audit_logs` (только `invoice.delete`/`invoice.archive`) — обнаружено тем же приёмочным прогоном. Не блокирует запуск (сама финансовая операция транзакционна и видна в `invoices`/`payments`/`ledger_entries`), но снижает полноту журнала для финансового раздела; стоит закрыть отдельной, небольшой фазой.
